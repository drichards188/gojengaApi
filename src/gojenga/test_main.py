...
def test_get_user():
    response = client.get("/handlers/kovax", headers={'Is-Test': 'True'})
...
def test_create_user():
    response = client.post("/handlers", json={"name": "kovax", "password": "5182"}, headers={'Is-Test': 'True'})
    assert response.json() == {"response": "insert item success"}


def test_update_user():
    response = client.put("/handlers/david", json={"name": "david", "password": "0000"}, headers={'Is-Test': 'True'})
...
def test_delete_user():
    ...
    response = client.delete("/handlers/zala", headers={'Is-Test': 'True'})
