...
@app.get("/")
async def use_oauth(token: str = Depends(oauth2_scheme)):
    return {"token": token}
...
@app.post("/login")
async def login(request: Request, data: User, is_test: Optional[bool] | None = Header(default=False)):
    with tracer.start_as_current_span(
            "login",
            context=extract(request.headers),
            attributes={'attr.username': data.name, 'attr.is_test': is_test},
        ...
        try:
            ...
            resp = UserHandler.handle_login(data.name, data.password, is_test)
            return {"response": resp}
        except Exception as e:
            logger.error(e)
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
